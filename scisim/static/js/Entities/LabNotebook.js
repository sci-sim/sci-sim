...
LabNotebook.prototype.addNoteField = function(e){
	var field = $("<li class='note-creator'><input class='form-control new-note-field'  type='text' name='newNote' placeholder='New Note'><button id='add' class='btn btn-default'>Add</button><button id='cancel' class='btn btn-default'>Cancel</button></li>");
	$('.labenotebook').append(field);
	$('.new-note-field').focus();
